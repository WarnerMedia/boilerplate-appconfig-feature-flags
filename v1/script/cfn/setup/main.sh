...
APPCONFIG_ACTION_MODE="CREATE_UPDATE"
...
echo ""
read -p "Application name (Default: $APPLICATION_NAME): " application_name
echo "The application name is: ${application_name:-$APPLICATION_NAME}"

# echo ""
# read -p "Application description (Default: $APPLICATION_DESCRIPTION): " application_description
# echo "The application description is: ${application_description:-$APPLICATION_DESCRIPTION}"
...
echo ""
echo "If you would like to set all of the CloudFormation actions (for AppConfig) to a different action mode, then set that region next."
read -p "CloudFormation Service Action Mode (Default: $APPCONFIG_ACTION_MODE): " appconfig_action_mode
echo "The CloudFormation Service Action Mode is: ${appconfig_action_mode:-$APPCONFIG_ACTION_MODE}"
...
if [ -n "${prod_profile:-$PROD_PROFILE}" ]; then
  ...
  run_deploy "${stack_name:-$STACK_NAME}$STACK_NAME_SUFFIX" "AppGitHubOrganization=${github_organization:-$GITHUB_ORGANIZATION} AppSourceRepository=${app_source_repository:-$APP_SOURCE_REPOSITORY} ExternalArtifactBucket=${nonprod_s3_artifact_bucket:-$NONPROD_S3_ARTIFACT_BUCKET} EnableCustomBuild=${prod_enable_custom_commands:-$PROD_ENABLE_CUSTOM_COMMANDS} ProjectName=${project_name:-$PROJECT_NAME} ApplicationName=${application_name:-$APPLICATION_NAME} ActionMode=${action_mode:-$ACTION_MODE} AppConfigActionMode=${appconfig_action_mode:-$APPCONFIG_ACTION_MODE} SecondRegion=${second_region:-$SECOND_REGION} UnstableEnvironment=NONE InitialEnvironment=NONE QaEnvironment=NONE StageEnvironment=NONE TagEnvironment=prod " "$TAGS environment=prod" "${prod_profile:-$PROD_PROFILE}"
...
if [ -n "${nonprod_profile:-$NONPROD_PROFILE}" ]; then
  ...
  run_deploy "${stack_name:-$STACK_NAME}$STACK_NAME_SUFFIX" "AppGitHubOrganization=${github_organization:-$GITHUB_ORGANIZATION} AppSourceRepository=${app_source_repository:-$APP_SOURCE_REPOSITORY} ExternalArtifactBucket=${prod_s3_artifact_bucket:-$PROD_S3_ARTIFACT_BUCKET} EnableCustomBuild=${nonprod_enable_custom_commands:-$NONPROD_ENABLE_CUSTOM_COMMANDS} ProjectName=${project_name:-$PROJECT_NAME} ApplicationName=${application_name:-$APPLICATION_NAME} ActionMode=${action_mode:-$ACTION_MODE} AppConfigActionMode=${appconfig_action_mode:-$APPCONFIG_ACTION_MODE} SecondRegion=${second_region:-$SECOND_REGION} ProdEnvironment=NONE TagEnvironment=nonprod " "$TAGS environment=nonprod" "${nonprod_profile:-$NONPROD_PROFILE}"
  if [ "${first_run:-$FIRST_RUN}" = "Yes" ]; then
